...
const statesData: State[] = [
  ...
  { id: 2, name: 'Arunachal Pradesh', image: state2, packages: '8', prices: '18,200' },
  { id: 3, name: 'Andaman & Nicobar', image: state3, packages: '5', prices: '20,000' },
  { id: 4, name: 'Himachal Pradesh', image: state4, packages: '10', prices: '22,500' },
  { id: 5, name: 'Karnataka', image: state5, packages: '7', prices: '12,800' },
  { id: 6, name: 'Jammu & Kashmir', image: state6, packages: '9', prices: '24,000' },
  ...
  { id: 11, name: 'Uttarakhand', image: state11, packages: '8', prices: '21,000' },
];
...
const StatesCarousel: React.FC = () => {
  ...
  useEffect(() => {
    const intervalId = setInterval(() => {
      setCurrentIndex((prevIndex) => (prevIndex + 1) % (statesData.length - statesToShow + 1));
    }, 3000);
          ...
          {statesData.map((state) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-black p-4 text-center flex bg-blue-500 rounded-lg rounded-t-none justify-between">
                <h2 className="text-white text-base font-semibold">{state.name}</h2>
                <div className="text-lg text-yellow-400 flex items-center font-semibold">
                  <span className="text-xs mr-2 font-thin">Starting price</span> {`₹${state.prices}`}
                </div>
              </div>
            </div>
          ))}
